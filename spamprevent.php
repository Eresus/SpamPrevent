	...
	function clientBeforeSend($text)
	{
		...
		if ($this->settings['href_method'] != 'none')
		{
			...
			for ($i = 0; $i < count($matches); $i++)
			{
				...
				{
					case 'onmouseover':
						$text = substr_replace($text, $this->settings['href_fake_email'],
							$matches[$i][1][1]+$delta, strlen($matches[$i][1][0]));
			...
			for ($i = 0; $i < count($matches); $i++)
			{
				if (!preg_match('/mailto:/i', $matches[$i][0][0]))
				{
					switch ($this->settings['text_method'])
					{
						case 'entity':
							$replace = '';
							for ($j = 0; $j < strlen($matches[$i][2][0]); $j++)
							{
								$replace .= '&#'.ord($matches[$i][2][0]{$j}).';';
							}
							$text = substr_replace($text, $replace, $matches[$i][2][1]+$delta,
								strlen($matches[$i][2][0]));
